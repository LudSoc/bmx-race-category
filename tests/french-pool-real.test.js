...
const hasData = fs.existsSync(path.join(__dirname, '..', '..', 'sqorz_stats', 'pilots-index.json'));
...
const norm = SC.norm;
const pilotsSrc = path.join(__dirname, '..', '..', 'sqorz_stats', 'pilots-index.json');
const intl = ['uec-index.json', 'uci-index.json', 'uci-worldcup-index.json']
  .map(f => path.join(__dirname, '..', '..', 'sqorz_stats', f))
